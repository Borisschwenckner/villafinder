...
$sort_name ='Price_descending';
  ...
  if (isset($_GET)) {
    ...
    if (isset($_GET["sort"])){
      $sort=  $_GET["sort"];
      $sort_field = explode('|', $sort)[0];
      $sort_direction = explode('|', $sort)[1];
      $sort_name = explode('|', $sort)[2];
  }
      ...
      <left><br>
        <table class="table1">
          ...
          <tr>
            ...
            <td data-label='<? echo $lang_tanslate['Provider:']; ?>'><select name="site" >
                ...
                while ($row_site = $statement_site->fetch(PDO::FETCH_ASSOC)) {
                  ...
                  echo '<option value="' . $row_site["site"] . '" '. ($row_site["site"]==$def_site ? 'selected="selected">' : '>') . $row_site["site"] . '</option>';
          ...
          <tr>
            ...
            <td><select name="region" id="region" >
                ...
                while ($row_area = $statement_area->fetch(PDO::FETCH_ASSOC)) {
                  echo '<option value="' . $row_area["area"] . '" '. ($row_area["area"]==$def_region ? 'selected="selected">' : '>') . $row_area["area"] . '</option>';
                }
                while ($row_region = $statement_region->fetch(PDO::FETCH_ASSOC)) {
                  echo '<option value="' . $row_region["name"]. '" '. ($row_region["name"]==$def_region ? 'selected="selected">' : '>') . $row_region["name"] . '</option>';
                }
                ?>
          </tr>
          ...
          <!-- Start multiple Orte --!>
            ...
            <? if ($multiple_locations == 0){  ?>
               <td data-label=<?echo $lang_tanslate['Location:'];?> ><select id="location" name="location" >
                <option selected="selected"><? echo "$def_location"; ?></option>
              <option value="Select">Select</option>
            ...
            <tr>
                ...
                <select name="type">
                  ...
                  while ($row_ptype = $statement_ptype->fetch(PDO::FETCH_ASSOC)) {
                    echo '<option value="' . $row_ptype["value"] . '" '. ($row_ptype["value"]==$def_type ? 'selected="selected">' : '>') . $row_ptype["value"] . '</option>';
        ...
        <table border=0 class="table2">
          ...
          <table border=0 class="table4">
            ...
            <tr><td data-label=<? echo $lang_tanslate['Sort:']; ?>><? echo $lang_tanslate['Sort:']; ?>
            <select  name="sort">
                <option selected="selected" value=<?echo $sort_field.'|'.$sort_direction.'|'.$sort_name;?>><? echo $lang_tanslate[$sort_name]; ?></option>
                <option value="create_date|desc|Date_descending"><? echo $lang_tanslate['Date_descending']; ?></option>
                <option value="create_date|asc|Date_ascending"><? echo $lang_tanslate['Date_ascending']; ?></option>
                <option value="price|asc|Price_ascending"><? echo $lang_tanslate['Price_ascending']; ?></option>
                <option value="price|desc|Price_descending"><? echo $lang_tanslate['Price_descending']; ?></option>
            </select>
            </td></tr>
              </table>
          ...
          <tr>
            <td class=right1 ><input type="submit" name="submit" value=<? echo $lang_tanslate['Search']; ?>>
  ...
  if (isset($_GET['locations'])){
    ...
    if ($favorites_only == 1)
      ...
      $sql = $sql . " order by $sort_field $sort_direction limit  :limit offset :offset;";
    ...
    #  echo ($sql);
      ...
      echo " " . $qty_results . " " .$lang_tanslate['Results'] ."</td></tr>";
      ...
      while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<td valign=top class='hidenormal'  ><a  href='$note_url''><img src='img/note.svg' class='filter-navy' width=30></a></td>";
        ...
        echo "<td class=hidemobile  width=200  align=top rowspan=6 ><a href=\"" . $row['url'] . "\" . ' ' . $target .><img src='", ($local_image_url), " ' width=200  /></td>
              <td class=hidemobile width=150><a href='$note_url''><img src='img/note.svg'  class='filter-navy' width=30></a></td></tr>"; //height=200
        ...
        echo "<tr><th>" . $lang_tanslate['Date'] . "</th><td colspan=3  width=600 >", date("d.m.Y", strtotime($row['create_date'])), " </td></tr>";
        if ($row['inactive_date'] != Null and $row['state'] == 'inactive') {
          echo "<tr><th>" . $lang_tanslate['Deactivated'] . "</th><td colspan=3  width=600 >", date("d.m.Y", strtotime($row['inactive_date'])), " </td></tr>";
        }
