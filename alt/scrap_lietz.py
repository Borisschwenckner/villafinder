...
if testmode == True:
    urls = ['https://www.lietzmallorca.com/mallorca.xhtml?id[obj0]=93']
    scraped =''
...
#        url = (url[0:pos_url])
    try:
    #if 1 > 0:
        if  url[-2].isdigit() and url  not in str(scraped)  :
            print (url)
            ...
            offer_type =''
            count = 1
            #tables = (soup.find_all("div", class_="details-desktop"))
            #htmltable = soup.find('table', { 'div' : 'details-desktop' })
            #print(htmltable)
            #table = soup.find('table', attrs={'div':'details-desktop'})
            #print(table)
            #tags = table.find_all("tr")
            #table = soup.find("table", { "div" : "details-desktop" })
            #print(tables)
            ...
            #for tag in tables:
                try:
                    ...
                    if (tag.find("td").get_text().strip())  =='ImmoNr':
