...
if testmode == True:
    urls = ['https://www.kensington-international.com/mallorca/de/immobilien/KNP07717']
    scraped =''
...
for url in urls:
    ...
    try:
    #if 1 > 0:
        if  url[-3].isdigit() and url  not in str(scraped)and '?' not in url :
            print (url)
            ...
            tags = (soup.find_all("div", class_="col-6 col-md-4 mb-3"))
            for tag in tags:
                #print (tag)
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Ref.':
                    ref= (tag.find("h5", class_="text-primary").get_text().strip())
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Objektart':
                    property_type = (tag.find("h5", class_="text-primary").get_text().strip())
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Kaufpreis':
                    price= (tag.find("h5", class_="text-primary").get_text().strip())
                    offer_type ='Buy'
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Kaltmiete':
                    price= (tag.find("h5", class_="text-primary").get_text().strip())
                    offer_type ='Rent'
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Wohnfläche':
                    size= (tag.find("h5", class_="text-primary").get_text().strip().replace('.','').replace('m²','').replace(',','.'))
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Grundstücksfläche':
                    groundsize= (tag.find("h5", class_="text-primary").get_text().strip().replace('.','').replace('m²','').replace(',','.'))
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Schlafzimmer':
                    bedroom= (tag.find("h5", class_="text-primary").get_text().strip())
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Badezimmer':
                    bathroom= (tag.find("h5", class_="text-primary").get_text().strip())
                if (tag.find("small", class_="text-muted").get_text().strip()) =='Region':
                    location= (tag.find("h5", class_="text-primary").get_text().strip())
            ...
            images = []
            for img in soup.findAll('a', class_='opengallerycenter'):
                #print(img.get('href'))
                try:
                    if  '/openimmo/data/'in str(img['href']) :
